...
(function () {
  ...
  function render() {
    ...
    if (state.view === "home") renderHub();
      ...
      if (window.ShotTracker) window.ShotTracker.render(state.view);
      else root.innerHTML = '<p class="empty-state">Shot tracker failed to load.</p>';
    }
  ...
  /* Shot tracker lives in shots.js (Phase 2 / Supabase). */
  ...
  window.addEventListener("hashchange", () => {
    const route = parseHash();
    if (window.ShotTracker && !String(route.view).startsWith("shots")) {
      window.ShotTracker.onLeave();
    }
